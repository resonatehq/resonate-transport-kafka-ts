...
import type {
	ResonateError,
	ResonateServerError,
} from "@resonatehq/sdk/dist/src/exceptions";
import exceptions from "@resonatehq/sdk/dist/src/exceptions";
import type {
	CallbackRecord,
	...
	ScheduleRecord,
	TaskRecord,
} from "@resonatehq/sdk/dist/src/network/network";
...
export class KafkaTransport implements Network, MessageSource {
				...
				if (res.error) {
					callback(
						exceptions.SERVER_ERROR(
							res.error.message,
							true,
							res.error as ResonateServerError,
						),
					);
				} else {
		...
		await this.consumerMessages.run({
			eachMessage: async ({ message }) => {
				...
				try {
					...
					if (
						(data?.type === "invoke" || data?.type === "resume") &&
						util.isTaskRecord(data?.task)
					) {
						msg = {
							type: data.type,
							task: data.task,
						...
						data?.type === "notify" &&
						util.isDurablePromiseRecord(data?.promise)
					) {
						msg = {
							type: data.type,
	...
	async send<T extends Request>(
		...
		this.pendingRequests.set(
			correlationId,
			callback as (err?: ResonateError, res?: Response) => void,
		);
...
function mapKafkaResponseToResponse({
	operation,
	response,
		...
		case "promises.callback":
			return {
				...
				callback: response.callback
					? convertCallback(response.callback)
					: undefined,
				promise: convertPromise(response.promise),
			};
